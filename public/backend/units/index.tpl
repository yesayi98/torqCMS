...
{block name="container-content"}
      ...
      {block name="buttons"}
        <div class="col-sm-3">
          <div class="btn-group float-sm-right">
            <a href="{url url='backend/units/detail'}" class="btn btn-success waves-effect waves-light m-1"><i class="fa fa-plus pr-2"></i>Add New</a>
          </div>
        </div>
      {/block}
    </div>
  ...
  {block name="table-block"}
    <div class="list-container" data-content='true'>
      {include file="backend/units/table.tpl"}
    </div>
  {/block}
{/block}
